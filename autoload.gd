...
var player_base_attack: int = 10 # Base attack stat for player
var player_base_defense: int = 5 # Base defense stat for player

# --- Inventory and Equipment ---
# Inventory: Dictionary mapping item_id (String) to quantity (int)
# Example: { "common_sword": 2, "rare_axe": 1 }
var player_inventory: Dictionary = {}

# Equipped items: Dictionary mapping slot_id (String) to item_id (String)
# Example: { "equip_slot_1": "common_sword", "equip_slot_2": "" }
# These slot IDs ("equip_slot_1", etc.) will correspond to your UI slots in GameMenu.
var player_equipped_items: Dictionary = {
	"equip_slot_1": "", # Default empty string for no item
	"equip_slot_2": "",
	"equip_slot_3": "",
	"equip_slot_4": "",
	# Add more slot IDs here if you have more than 4 equip slots in your UI (e.g. for SubUpgradeStat nodes)
}
...
# --- Signals ---
# Updated: Signal now emits item_id and new_count
...
func _ready() -> void:
	...
	# Check if a save file was loaded. If not, and inventory is empty, add default items.
	if player_inventory.is_empty() and player_coins == 0: # Simple check for a new game scenario
		...
		# Add any other items you want to start with for testing.
		# IMPORTANT: Ensure these item IDs exist in your Item.tres files and are listed in ItemData.gd's ITEM_PATHS.
		player_coins = 100
...
func reset_variables():
	...
	bullet_speed = 500
	score = 0
	# You might want to add lines here to reset inventory/equipment if this function
	# is used to start a completely new game, e.g.:
	# player_inventory.clear()
	# player_equipped_items = { "equip_slot_1": "", "equip_slot_2": "", "equip_slot_3": "", "equip_slot_4": "" }
	# save_all_player_data()


# --- Unified Player Data Save/Load ---
# All player-specific data (coins, inventory, equipped items) will be saved to one file
...
# IMPORTANT: This function now expects the 'id' (String) of your Item resource (e.g., "common_sword")
func add_item_to_inventory(item_id: String, quantity: int = 1) -> void:
	...
	# Emit the signal with the updated item_id and new total count
...
func remove_item_from_inventory(item_id: String, quantity: int = 1) -> bool:
	if player_inventory.has(item_id) and player_inventory[item_id] >= quantity:
		...
		save_all_player_data() # Save after every change
		return true
	print("INVENTORY ERROR: Cannot remove ", quantity, " of ", item_id, ". Not enough in inventory.")
	return false

# NEW: Get item count (useful for UI)
func get_item_count(item_id: String) -> int:
	return player_inventory.get(item_id, 0)

# NEW: Get a copy of the inventory (to avoid direct external modification)
func get_all_inventory_items() -> Dictionary:
	return player_inventory.duplicate()


# --- Coin Management ---
# This function now uses the unified save_all_player_data()
